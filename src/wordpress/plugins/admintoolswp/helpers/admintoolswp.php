...
class AdminToolsWP
{
	...
	public static function adminMenu()
	{
		...
		self::$menu_page = add_menu_page(
			...
			array('AdminToolsWP', 'boot'),
			plugins_url('app/media/images/akeeba-admintools-wp-small-16.png', self::$absoluteFileName)
		);

		add_action('load-'.self::$menu_page, array('AdminToolsWP', 'add_options'));
	}
	...
	{
		$allowed = array('admintoolswp_per_page');
	...
	/**
	 * Starts output buffer in the admin area. Usually this is not required, since the plugin will start the buffering for us.
	 * However the plugin could be manually disabled, so I need a fallback to be 100% sure to have output buffering on
	...
	public static function startAdminBuffer()
	{
		...
		if (isset($_REQUEST['page']) && ($_REQUEST['page'] == $page) && !defined('ADMNITOOLSWP_OBFLAG'))
		{
			...
			@ob_start(array('AdminToolsWP', 'clearAdminBuffer'));
		}
	}
