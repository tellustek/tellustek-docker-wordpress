...
namespace Akeeba\AdminTools\Library\Transfer;
...
class Transfer
{
	...
	public function setAdapter($className)
	{
		...
		elseif (class_exists('\\Akeeba\\AdminTools\\Library\\Transfer\\Adapter\\' . ucfirst($className)))
		{
			$className = '\\Akeeba\\AdminTools\\Library\\Transfer\\Adapter\\' . ucfirst($className);
			$adapter = new $className;
		}

		if (is_object($adapter) && ($adapter instanceof TransferInterface))
		{
			$this->adapter = $adapter;
		}
	}
	...
	public function getAdapterName()
	{
		if(is_object($this->adapter))
		{
			...
			return strtolower(str_ireplace('Akeeba\\AdminTools\\Library\\Transfer\\Adapter\\', '', $class));
		}
	...
	/**
	 * Sends a request to a remote URL
	 *
	 * @param   string  $url
	 ...
	 * @return bool|mixed
	 */
	public function sendToURL($url, $postdata = array())
	{
		try
		{
			return $this->adapter->sendData($url, $postdata, $this->adapterOptions);
		}
		catch (\RuntimeException $e)
		{
			return false;
			...
			do
			{
				...
				if ($result !== false)
				{
					...
					if ($fp === false)
					{
						...
						$retArray['status'] = false;
						$retArray['error'] = sprintf('Local file %s is not writeable', $localFilename);
	...
	protected static function getFiles($path, array $ignoreFolders = array(), array $ignoreFiles = array())
	{
		...
		foreach ($files as $file)
		{
			...
			$return[] = array(
				'fullpath'  => $file,
				'classname' => '\\Akeeba\\AdminTools\\Library\\Transfer\\Adapter\\' . ucfirst(basename($parts[0], '.php'))
			);
		}
