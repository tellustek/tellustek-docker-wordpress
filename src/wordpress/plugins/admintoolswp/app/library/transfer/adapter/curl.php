 ...
 */
class Curl extends AbstractAdapter implements TransferInterface
{
	...
	public function downloadAndReturn($url, $from = null, $to = null, array $params = array())
	{
		...
		if ($result === false)
		{
			$error = sprintf('The download failed: cURL error %s: %s', $errno, $errmsg);
		}
		...
		elseif ($http_status > 399)
		{
			$result = false;
			$errno = $http_status;
			$error = sprintf('Unexpected HTTP status %s', $http_status);
		}
	...
	public function sendData($url, array $postdata = array(), array $params = array())
	{
		...
		if ($result === false)
		{
			$error = sprintf('Transfer failed: cURL error %s: %s', $errno, $errmsg);
		}
		elseif ($http_status > 399)
		{
			$result = false;
