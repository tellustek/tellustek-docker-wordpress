...
namespace Akeeba\AdminTools\Library\Transfer\Adapter;

use Akeeba\AdminTools\Library\Transfer\TransferInterface;
 ...
 */
class Fopen extends AbstractAdapter implements TransferInterface
{
	 ...
	 *
	 * @throws  \RuntimeException  A generic exception is thrown on error
	 */
	public function downloadAndReturn($url, $from = null, $to = null, array $params = array())
	{
		...
		{
			$error = 'The download failed: fopen returned no header information';
		...
		else
		{
			...
			if ($http_code >= 299)
			{
				$error = sprintf('Unexpected HTTP status %s', $http_code);
				throw new \RuntimeException($error, $http_code);
			}
		}

		if ($result === false)
		{
			$error = sprintf('The download failed: fopen returned no header information');
	...
	public function sendData($url, array $postdata = array(), array $params = array())
	{
		$options = array(
			'http'	=> array(
				'method'	=> 'GET',
