...
namespace Akeeba\AdminTools\Library\Autoloader;
		...
		if (isset($this->prefixLengths[$first]))
		{
			foreach ($this->prefixLengths[$first] as $prefix => $length)
			{
				if (0 === strpos($class, $prefix))
				{
					...
					{
						// All the files inside the Admin Tools package are lowercase. So I have to lowercase the full path
						// BUT NOT THE ORIGINAL DIRECTORY, since it could contain uppercase chars
						$file = $dir . DIRECTORY_SEPARATOR . strtolower(substr($logicalPath, $length));
...
Autoloader::getInstance()->addMap('Akeeba\AdminTools\Library\\', array(realpath(__DIR__ . '/..')));
