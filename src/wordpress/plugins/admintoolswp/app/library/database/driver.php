...
abstract class Driver implements DatabaseInterface
{
	...
	public static function getConnectors($tech = null)
	{
		...
		foreach ($iterator as $file)
		{
			...
			$class = str_ireplace('.php', '', '\\Akeeba\\AdminTools\\Library\\Database\\Driver\\' . ucfirst(trim($fileName)));
	 ...
	 *
	 * @param   array $options Parameters to be passed to the database driver or a DI Container to read app
	 *                                   config from.
	 *
	...
	{
		// Default settings for missing option indexes
		$default = array(
			'driver'	 => 'mysqli',
			'database'	 => '',
			'host'		 => '',
		...
		if (empty(self::$instances[$signature]))
		{
			...
			$class = '\\Akeeba\\AdminTools\\Library\\Database\\Driver\\' . ucfirst(strtolower($options['driver']));
	...
	public function getQuery($new = false)
	{
		...
		{
			// Derive the class name from the driver.
			$class = '\\Akeeba\\AdminTools\\Library\\Database\\Query\\' . ucfirst($this->name);
