...
class Phptokenizer
{
    ...
    protected function findToken($type, $name, $skip = 0)
    {
        ...
        foreach($collection as $token)
        {
			...
			// How should we compare the value? If it's an encapsed string, we should wrap the value between single or double quotes,
			// in other cases simply check the original value
			if ($info['token'] != 'T_CONSTANT_ENCAPSED_STRING')
			{
				if ($info['value'] != $name)
				{
					continue;
			...
			if ($info['token'] == 'T_CONSTANT_ENCAPSED_STRING')
			{
				// Value doesn't match the original, the single quoted and double quoted name. Let's skip it
				if (($info['value'] != $name) && ($info['value'] != "'".$name."'") && $info['value'] != '"'.$name.'"')
				{
					continue;
				}
			}
			...
			while(!is_array($next))
			{
				...
				// The next token is not an array (ie it's a char like ;.=?)? Move the iterator forward and fetch the next one
				if(!is_array($next))
				{
				...
				// Just for debug
				$token_name = token_name($next[0]);

				return $next[2] + $offset - 1;
			}
        }
