...
namespace Akeeba\AdminTools\Library\Html;
...
abstract class Select
{
	 ...
	 */
	public static function booleanList($name, $attribs = [], $selected = null, $yes = 'AWF_YES', $no = 'AWF_NO', $id = false)
	{
		$arr = [static::option('0', Language::_($no)), static::option('1', Language::_($yes))];
	...
	{
		// Set default options
		$options = array_merge(static::$formatOptions, ['format.depth' => 0, 'id' => false]);
	 ...
	 * @return  string  HTML for the select list
	 */
	public static function suggestionList($data, $idTag = null, $optKey = 'value', $optText = 'text', $translate = false)
	{
		// Set default options
		$options = array_merge(static::$formatOptions, ['format.depth' => 0, 'id' => false]);
	...
	{
		// Set default options
		$options = array_merge(static::$formatOptions, ['format.depth' => 0, 'option.format' => '', 'id' => null]);
	                                 ...
	                                 $translate = false)
	{
		...
		if (isset($attribs['classMaps']))
		{
			$classMaps = (array) $attribs['classMaps'];
			unset($attribs['classMaps']);
		}
		...
		foreach ($data as $optionObject)
		{
			...
			$labelText   = $translate ? Language::_($optionObject->$optText) : $optionObject->$optText;
			...
			if (array_key_exists($optionObject->$optKey, $classMaps))
			{
				$labelExtra .= sprintf(' class="%s"', $classMaps[$optionObject->$optKey]);
			}
			...
			if ($forToggle)
			{
				$html .= "\n\t" . '<input type="radio" name="' . $name . '" id="' . $id . '" value="' . $optionValue . '" ' . $extra
					. $attribs . ' />';
				$html .= "\n\t" . '<label for="' . $id . '" id="' . $id . '-lbl" ' . $labelExtra . '>' . $labelText . '</label>';
			}
			else
			{
	...
	public static function booleanswitch($name, $selected, array $attribs = [])
	{
		...
		$html .= '<label for="' . $name . '-2" class="green">' . Language::_('YES') . '</label>';
		$html .= '<input type="radio" class="radio-no" name="' . $name . '" ' . $checked_1 . 'id="' . $name . '-1" value="0">';
		$html .= '<label for="' . $name . '-1" class="red">' . Language::_('NO') . '</label>';
		$html .= '</div>';
