...
<form id="admintoolswpForm" method="get" class="akeeba-form">
    ...
    <input type="hidden" name="view" value="AutoBannedAddresses" />
    <?php wp_nonce_field('getAutoBannedAddresses', '_wpnonce', false) ?>
            ...
            <?php foreach($this->items as $item):?>
                <tr>
                    <td class="check-column">
                        <input id="cb-select-<?php echo str_replace(['.',':'], ['-','-'], $item->ip) ?>" type="checkbox" name="cid[]" value="<?php echo $item->ip?>" />
                    </td>
