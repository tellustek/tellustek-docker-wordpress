                    ...
                    <tr>
						...
						<td>
							<a href="<?php echo ADMINTOOLSWP_URL; ?>&view=Redirections&task=edit&id=<?php echo $item->id?>">
								<?= site_url() ?>/<strong><?= $item->dest ?></strong>
							</a>
						</td>
                        <td>
							<a href="<?= (strstr($item->source, '://') ? $item->source : '../' . $item->source) ?>"target="_blank">
								<?= htmlentities($item->source) ?>&nbsp;<img src="<?=ADMINTOOLSWP_MEDIAURL?>/app/media/images/external-icon.gif" border="0"  alt=""/>
							</a>
                        </td>
                        <td>
                        <?php
